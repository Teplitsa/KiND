...
gulp.task( 'build-css', function() {
	...
	var vendorFiles = gulp.src('.', {allowEmpty: true}),//gulp.src( [] ), // Components
		appFiles = gulp.src( basePaths.src + 'sass/style.scss' ) // Main file with @import-s
			.pipe( plugins.sass( {
...
gulp.task( 'build-editor-css', function() {
	...
	paths.push( basePaths.npm + 'modularscale-sass/stylesheets' );
	var appFiles = gulp.src( basePaths.src + 'sass/editor.scss' )
			.pipe( plugins.sass( {
...
gulp.task( 'full-build-css',
	gulp.series( 'build-css', 'build-blocks-css', 'build-gutenberg-css', 'build-editor-css' )
);
...
gulp.task( 'full-build',
	gulp.series( 'full-build-css', 'full-build-js' )
);
